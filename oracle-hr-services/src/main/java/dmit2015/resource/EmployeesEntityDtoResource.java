...
public class EmployeesEntityDtoResource {
    ...
    public Response updateEmployeesEntity(@PathParam("id") Integer id, EmployeesEntityDto dto) {
        if (!id.equals(dto.getEmployeeId())) {
            throw new BadRequestException();
        }

        EmployeesEntity existingEmployeesEntity = _employeesEntityRepository
                .findOptional(id)
                .orElseThrow(NotFoundException::new);

        EmployeesEntity updatedEmployeesEntity = EmployeesEntityMapper.INSTANCE.toEntity(dto);

        String errorMessage = BeanValidator.validateBean(EmployeesEntity.class, updatedEmployeesEntity);
        if (errorMessage != null) {
            return Response
                    .status(Response.Status.BAD_REQUEST)
                    .entity(errorMessage)
        ...
        return Response.ok(existingEmployeesEntity).build();
    }
    ...
    public Response delete(@PathParam("id") Integer employeesEntityId) {
        ...
        EmployeesEntity existingEmployeesEntity = _employeesEntityRepository
                .findOptional(employeesEntityId)
                .orElseThrow(NotFoundException::new);

        try {
            _employeesEntityRepository.remove(existingEmployeesEntity);    // Removes the EmployeesEntity from being persisted
        } catch (Exception ex) {
            // Return a HTTP status of "500 Internal Server Error" containing the exception message
            return Response
                    .serverError()
